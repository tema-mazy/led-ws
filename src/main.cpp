...
void drops() {
	...
	byte dothue = 0;
	for (int i = 0; i < 8; i++) {
		leds[beatsin16(i + 7, 0, NUM_LEDS - 1)] |= CHSV(dothue, 200,
				globalBrightness);
		dothue += 16;
	}
}
...
void bpm() {
	...
	for (int i = 0; i < NUM_LEDS; i++) {
		leds[i] = ColorFromPalette(gCurrentPalette, gHue + (i * 2),
				beat - (i * 10));
	}
}
...
void noise8_pal() {
	...
	for (int i = 0; i < NUM_LEDS; i++) { // Just ONE loop to fill up the LED array as all of the pixels change.
		...
		leds[i] = ColorFromPalette(gCurrentPalette, index, globalBrightness,
...
void brightnessRoutine() {
	if (brightnessDirection) {
		breathBrightness += 1;
		if (breathBrightness > globalBrightness + 32
				|| breathBrightness > 254) {
...
const char _ETPL[] PROGMEM
		= "<li><label for='e%d'><input type='checkbox' value='1' id='e%d' name='e%d' %s><span>%s</span></label></li>";
...
void handleRoot() {
	...
	page.replace("{state}",
			String(
					(state == 0) ?
							"<b style='color:red;'>On</b>" :
							"<b style='color:blue;'>Off</b>"));
	page.replace("{statebage}", String((state == 1) ? "on" : "off"));
	page.replace("{newstate}", String((state == 1) ? "off" : "on"));

	String sEff = "<ul class=\"effects-list\">";
	char c[255];

	for (int i = 0; i < ARRAY_SIZE(gPatterns); i++) {

		sprintf_P(c, _ETPL, i, i, i,
